...
public class LocalSemanticMemory {
    ...
    /**
     * 入力されたノイズ(Δin)と、データベース内の全過去ログ(Sp)から、
     * 外部APIに依存せず「システム独自の計算」で意味的に近い記憶を抽出する。
     */
    public List<StateTransaction> recallEpisodicMemory(String currentNoise, List<StateTransaction> allHistory, int limit) {
        ...
        PriorityQueue<MemoryScore> queue = new PriorityQueue<>(Comparator.comparingDouble(m -> m.score));
