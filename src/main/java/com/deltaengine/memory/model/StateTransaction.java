package com.deltaengine.memory.model;
...
public record StateTransaction(
  ...
  /** 次のクロック（Sn+1）のステートを生成する純粋関数。 前のトランザクションIDを自動で引き継ぎ、システム上の連続性（自意識のチェーン）を強制する。 */
  public StateTransaction applyDelta(
      String newExternalDelta, String newInternalState, String newOutputDelta) {
