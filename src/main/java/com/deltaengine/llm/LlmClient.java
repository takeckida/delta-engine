...
public class LlmClient {
  ...
  public LlmResponse computeSyncState(
      String previousState, String augmentedExternalDelta, String memoryContext) {
    String prompt =
        String.format(
            """
           【システムアーキテクチャ要件（差異モデル）】
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (response.statusCode() == 429) {
          System.err.printf(
              "\n[API Rate Limit (429)] クオータ超過。%d秒待機後に再試行します... (%d/%d)%n",
              (retryWaitMs / 1000), attempt, maxRetries);
